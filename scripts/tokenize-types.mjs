 ...
 * in core type definition files with theme token references.
 *
 * Run: node scripts/tokenize-types.mjs
 */
import fs from 'fs';
...
// Same snap logic as tokenize-styles.mjs
const spacingSnap = {
...
function processFile(filePath) {
  ...
  // Replace padding/margin/gap numeric values
  content = content.replace(
    /(\b(?:padding|paddingTop|paddingRight|paddingBottom|paddingLeft|paddingX|paddingY|paddingVertical|paddingHorizontal|margin|marginTop|marginRight|marginBottom|marginLeft|marginX|marginY|marginVertical|marginHorizontal|gap|rowGap|columnGap))(\s*:\s*)(\d+)(\s*[,}\n])/g,
  ...
  // Replace borderRadius numeric values
  content = content.replace(
  ...
  // Replace fontWeight numeric values
  content = content.replace(
    /(\bfontWeight)(\s*:\s*)(\d+)(\s*[,}\n])/g,
    (match, prop, sep, val, after) => {
  ...
  // Replace string fontWeight like '500'
  content = content.replace(
    /(\bfontWeight)(\s*:\s*)'(\d+)'/g,
    (match, prop, sep, val) => {
      const token = weightMap[Number(val)];
    ...
    if (tokensNeeded.length > 0) {
      ...
      if (!content.includes("from '../theme/create-theme'")) {
        ...
        if (lastImportEnd > 0) {
          ...
          // No imports found, add at top (after any initial comment)
...
console.log(`\nTypes: ${modified}/${files.length} files modified`);
