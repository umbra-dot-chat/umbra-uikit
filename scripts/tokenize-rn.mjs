 ...
 * in React Native component files with theme token references.
 *
 * Uses static imports (defaultSpacing/defaultRadii/defaultTypography)
 * since many of these values are in static style objects outside of
 * component render functions.
 *
 * Run: node scripts/tokenize-rn.mjs
 */
import fs from 'fs';
...
function processFile(filePath) {
  ...
  // All spacing-related props including RN-specific ones
  content = content.replace(
    /(\b(?:padding|paddingTop|paddingRight|paddingBottom|paddingLeft|paddingVertical|paddingHorizontal|margin|marginTop|marginRight|marginBottom|marginLeft|marginVertical|marginHorizontal|gap|rowGap|columnGap))(\s*:\s*)(\d+)(\s*[,}\n])/g,
  ...
  content = content.replace(
    /(\bborder(?:Top|Bottom)(?:Left|Right)Radius)(\s*:\s*)(\d+)(\s*[,}\n])/g,
    (match, prop, sep, val, after) => {
      const num = Number(val);
      if (num === 0) return match;
      const token = getRadiiToken(num);
  ...
  // fontWeight string like '500' or '600' (common in RN)
  content = content.replace(
    /(\bfontWeight)(\s*:\s*)'(\d+)'/g,
    (match, prop, sep, val) => {
      const token = weightMap[Number(val)];
  ...
  // fontWeight as string 'bold', 'normal' etc - skip these RN specific strings
...
// ---------------------------------------------------------------------------
// Main — process all RN source files (not tests, not stories)
// ---------------------------------------------------------------------------
...
const rnDir = path.join(ROOT, 'packages/react-native/src');
const files = walkDir(rnDir);
...
console.log(`\nReact Native: ${modified}/${files.length} files modified`);
